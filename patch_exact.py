...
for i, line in enumerate(lines):
    if "actions = {" in line:
        start_idx = i
    if start_idx != -1 and i > start_idx and "IconButton(onClick = {" in line and "openTtsSettingsSheet(activity, mainText, bookTitle)" not in "".join(lines[start_idx:i]) and "isSettingsOpen = true" in "".join(lines[i:i+10]):
        # actually, the last button is "isSettingsOpen = true". Let's look for it
        if "isSettingsOpen = true" in "".join(lines[i:i+10]):
            end_idx = i
            break
