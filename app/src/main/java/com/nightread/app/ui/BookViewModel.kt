...
class BookViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun sortBooks(books: List<BookEntity>, option: String = _sortOption.value): List<BookEntity> {
        return when (option) {
            ...
            com.nightread.app.data.SettingsManager.SORT_DATE_ASC -> books.sortedWith(compareBy<BookEntity> { if (it.lastReadTime > 0) it.lastReadTime else it.dateAdded }.thenBy { it.dateAdded }.thenBy { it.title })
            com.nightread.app.data.SettingsManager.SORT_DATE_DESC -> books.sortedWith(compareByDescending<BookEntity> { if (it.lastReadTime > 0) it.lastReadTime else it.dateAdded }.thenByDescending { it.dateAdded }.thenBy { it.title })
